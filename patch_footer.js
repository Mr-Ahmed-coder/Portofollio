const fs = require('fs');
const file = 'frontend/src/components/Footer.jsx';
let content = fs.readFileSync(file, 'utf8');

const newSocials = `  const socials = [
...
// Replace socials array
const arrayRegex = /\s*const socials = \[\s*\{\s*icon:\s*"f"[\s\S]*?\];/;
content = content.replace(arrayRegex, '\n' + newSocials);

// Replace a tag href to include target blank
content = content.replace(/<a key=\{i\} href=\{s\.href\} aria-label=\{s\.label\} style=\{\{/g, '<a key={i} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label} style={{');
